...
import { AiAgentPanel } from "./ai-agent-drawer";
 ...
 *
 * When the agent is closed the workspace owns the full width; when it is opened
 * from "Ask HireFlow AI" or from any open requirement the layout splits into a
 * two-column grid with the agent as an embedded, sticky right-hand panel that
 * fits inside the app card and scrolls its own conversation while the workspace
 * scrolls beside it. All requirement status is read through the verification
 * store, so a verification made in the panel updates the coverage ring, the
 * open-gap count, the evaluation report and the audit trail here immediately —
  ...
  return (
    <>
      <div
        className={cn(
          "items-start gap-4",
          chatOpen && "lg:grid lg:grid-cols-[minmax(0,1fr)_390px]",
        )}
      >
